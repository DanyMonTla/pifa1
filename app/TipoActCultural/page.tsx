// Catalogo: Tipo Actividad Cultural
'use client';
import React, { useState } from 'react';

interface TipoActividadCultural {
    id: number;
    tipo: string;
}

export default function TipoActividadCulturalABC() {
    const [lista, setLista] = useState<TipoActividadCultural[]>([]);
    const [tipo, setTipo] = useState('');
    const [seleccionado, setSeleccionado] = useState<number | null>(null);

    const handleAgregar = () => {
    if (!tipo.trim()) return;
    const nuevo: TipoActividadCultural = {
        id: Date.now(),
        tipo,
    };
    ...
    const seleccionarFila = (item: TipoActividadCultural) => {
    ...
    <div style={{ maxWidth: '800px', margin: '2rem auto' }}>
        <h2 style={{ textAlign: 'center' }}>Catálogo Tipo Actividad Cultural</h2>
